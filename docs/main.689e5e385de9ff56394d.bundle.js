...
`];const languageConverter={fromAttribute:resolveAlias},rangeConverter={fromAttribute:value=>value.split(",").map(s=>s.split("-").map(Number))};let code_highlight_CodeHighlightElement=class CodeHighlightElement extends lit_element.a{constructor(){super(...arguments),this.plain=!1}get codeElement(){let el=this.querySelector("code");return!el&&this.appendChild(el=document.createElement("code")),el}get source(){return this.codeElement.innerHTML.replace(/&nbsp;/gim," ").replace(/&amp;/gim,"&").replace(/&quot;/gim,'"').replace(/&lt;/gim,"<").replace(/&gt;/gim,">").replace(/<br>/gm,"\n")}connectedCallback(){super.connectedCallback(),new MutationObserver(records=>{const{codeElement:codeElement}=this;!!records.find(record=>record.target===codeElement)&&this.requestUpdate()}).observe(this,{attributes:!1,childList:!0,subtree:!0})}render(){var _a;const{language:language="clike",source:source}=this;return lit_element.d`
    <pre>
...
//# sourceMappingURL=main.689e5e385de9ff56394d.bundle.js.map